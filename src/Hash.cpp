...
{
  uint32_t* calccrc32Table = Crc32Table::getInstance().gCrc32Table;
  uint32_t* crc32Table = poly8_lookup;

  uint8_t* data = ( uint8_t* ) buf;
  while( len-- > 0 ) {
    uint8_t lookup = mHash ^ *(data++);
    mHash = crc32Table[lookup] ^ ( mHash >> 8 );
  }
}
...
HashCode Crc32::finalize()
{
  ...
  uint8_t values[4];
  values[0] = (mHash >> 24) & 0xFF;
  values[1] = (mHash >> 16) & 0xFF;
  values[2] = (mHash >>  8) & 0xFF;
  values[3] = (mHash >>  0) & 0xFF;
