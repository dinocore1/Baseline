...
int VectorImpl::insertVectorAt( const VectorImpl& vector, size_t index )
{
  return insertArrayAt( vector.arrayImpl(), index, vector.size() );
}

int VectorImpl::appendVector( const VectorImpl& vector )
{
  return insertVectorAt( vector, size() );
}

int VectorImpl::insertArrayAt( const void* array, size_t index, size_t length )
{
  if( index > size() ) {
    return BAD_INDEX;
  }
...
int VectorImpl::appendArray( const void* array, size_t length )
{
  return insertArrayAt( array, size(), length );
}

int VectorImpl::insertAt( size_t index, size_t numItems )
{
  return insertAt( 0, index, numItems );
}

int VectorImpl::insertAt( const void* item, size_t index, size_t numItems )
{
  if( index > size() ) {
    return BAD_INDEX;
  }
...
int VectorImpl::add( const void* item )
{
  return insertAt( item, size() );
}

int VectorImpl::replaceAt( size_t index )
{
  return replaceAt( 0, index );
}

int VectorImpl::replaceAt( const void* prototype, size_t index )
{
...
int VectorImpl::removeItemsAt( size_t index, size_t count )
{
...
int VectorImpl::setCapacity( size_t new_capacity )
{
...
int SortedVectorImpl::indexOf( const void* item ) const
{
...
int SortedVectorImpl::_indexOrderOf( const void* item, size_t* order ) const
{
  // binary search
  int err = NAME_NOT_FOUND;
  int l = 0;
  int h = size() - 1;
  int mid;
...
int SortedVectorImpl::add( const void* item )
{
  size_t order;
  int index = _indexOrderOf( item, &order );
  if( index < 0 ) {
...
int SortedVectorImpl::merge( const VectorImpl& vector )
{
  // naive merge...
  if( !vector.isEmpty() ) {
    ...
    for( size_t i = 0 ; i < s ; i++ ) {
      int err = add( reinterpret_cast<const char*>( buffer ) + i * is );
      if( err < 0 ) {
        return err;
      }
    }
...
int SortedVectorImpl::merge( const SortedVectorImpl& vector )
{
...
int SortedVectorImpl::remove( const void* item )
{
  int i = indexOf( item );
  if( i >= 0 ) {
